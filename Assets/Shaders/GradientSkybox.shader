...
Shader "GradientSkybox"
{
	Properties
	{
		...
		[ASEBegin]_TopColor("TopColor", Color) = (0.5179334,0.8615229,0.9716981,1)
		[ASEEnd]_BottomColor("BottomColor", Color) = (0.3882353,0.2117647,0.5019608,1)
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 ShadowCoords = float4( 0, 0, 0, 0 );
					...
					#endif
				#endif
				float3 normalizeResult38 = normalize( WorldPosition );
				float4 lerpResult12 = lerp( _TopColor , _BottomColor , pow( (0.0 + (normalizeResult38.y - 0.0) * (1.0 - 0.0) / (1.0 - 0.0)) , 0.98 ));
				float4 Sky26 = lerpResult12;
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = Sky26.rgb;
				float Alpha = 1;
...
ASEEND*/
//CHKSM=B54A209BEF907A843749F3A267C999FA84163942